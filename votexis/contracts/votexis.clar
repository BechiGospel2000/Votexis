...
(define-map community-motions
  { motion-id: uint }
  {
    ...
    voting-begins: uint,
    voting-ends: uint,
    current-status: (string-ascii 32),  ;; "draft", "active", "passed", "rejected", "executed"
    motion-category: (string-ascii 32), ;; "parameter", "upgrade", "fund", "text"
    required-majority: uint,            ;; Out of 10000 (e.g., 6000 = 60%)
    min-participation: uint,            ;; Out of 10000 (e.g., 1000 = 10%)
    yes-votes: uint,
    no-votes: uint,
    abstain-votes: uint
  }
...
(define-map motion-actions
  { motion-id: uint, action-id: uint }
  {
    action-category: (string-ascii 32), ;; "set-parameter", "transfer-funds"
...
;; Voting records
(define-map ballot-records
  { motion-id: uint, voter: principal }
  {
    ballot-choice: (string-ascii 16), ;; "yes", "no", "abstain"
    voting-power: uint,
    vote-timestamp: uint
  }
)
...
(define-private (convert-string-to-uint (str (string-utf8 256)))
  (if (is-eq str u"1440") (some u1440)
      (if (is-eq str u"10080") (some u10080)
          (if (is-eq str u"2880") (some u2880)
              (if (is-eq str u"100000000000") (some u100000000000)
                  (if (is-eq str u"1000") (some u1000)
                      (if (is-eq str u"5000") (some u5000)
                          (some u0)))))))
)
...
(define-public (create-governance-motion
  ...
  (let
    ((motion-id (var-get motion-counter))
     ...
     (deposit (var-get min-motion-deposit)))
...
(define-public (add-motion-action
                ...
                (transfer-amount (optional uint)))
  (let
    ((motion (unwrap! (map-get? community-motions { motion-id: motion-id }) (err u"Motion not found"))))
...
(define-public (cast-ballot
  ...
  (let
    ((motion (unwrap! (map-get? community-motions { motion-id: motion-id }) (err u"Motion not found")))
     (existing-ballot (map-get? ballot-records { motion-id: motion-id, voter: tx-sender }))
    ...
    ;; Remove previous ballot if exists
...
(define-public (update-protocol-setting (setting-key (string-ascii 64)) (setting-value (string-utf8 256)))
  (begin
    ...
    (match (map-get? protocol-settings { setting-key: setting-key })
      setting (begin
                (map-set protocol-settings
                  { setting-key: setting-key }
                  {
...
(define-read-only (get-motion-details (motion-id uint))
  (ok (unwrap! (map-get? community-motions { motion-id: motion-id }) (err u"Motion not found")))
)

(define-read-only (get-protocol-setting (setting-key (string-ascii 64)))
  (ok (unwrap! (map-get? protocol-settings { setting-key: setting-key }) (err u"Setting not found")))
)

(define-read-only (get-ballot-details (motion-id uint) (voter principal))
  (ok (unwrap! (map-get? ballot-records { motion-id: motion-id, voter: voter }) (err u"Ballot not found")))
)

(define-read-only (check-motion-status (motion-id uint))
  (match (map-get? community-motions { motion-id: motion-id })
